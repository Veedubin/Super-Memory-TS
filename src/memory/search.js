...
export class MemorySearch {
     ...
     */
    async tieredSearch(question, options) {
        ...
        if (vectorResults.length > 0) {
            const topScore = vectorResults[0]._similarity ?? 0;
     ...
     */
    async vectorOnlySearch(question, options) {
     ...
     */
    async textOnlySearch(question, options) {
     ...
     */
    async textSearchInternal(query, limit) {
        await this.ensureFuseIndex();
        if (!this.fuse) {
     ...
     */
    mergeVectorAndTextResults(vectorResults, textResults, limit) {
        const seen = new Set();
        const merged = [];
 ...
 */
export async function initializeSearch(dbUri) {
