...
// ==================== Validation ====================
const VALID_PRECISIONS = ['fp32', 'fp16', 'q8', 'q4'];
const VALID_DEVICES = ['auto', 'gpu', 'cpu'];
...
export function generateProjectId(projectPath) {
    ...
    if (envId)
        return sanitizeProjectId(envId);
...
function parseEnvConfig() {
    return {
        model: {
            precision: process.env[ENV_VARS.BOOMERANG_PRECISION] || DEFAULT_CONFIG.model.precision,
            device: process.env[ENV_VARS.BOOMERANG_DEVICE] || DEFAULT_CONFIG.model.device,
        ...
        logging: {
            level: process.env[ENV_VARS.BOOMERANG_LOG_LEVEL] || DEFAULT_CONFIG.logging.level,
        },
        performance: {
