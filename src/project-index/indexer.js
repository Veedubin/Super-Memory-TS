...
export class ProjectIndexer extends EventEmitter {
    ...
    pendingEventCount = 0;
    processingCompleteResolve = null;
    processingCompletePromise = null;
    ...
    lastMemoryWarning = 0;
    MEMORY_WARNING_COOLDOWN_MS = 5000; // Only warn every 5 seconds
    // Progress tracking
    progressCallbacks = {};
    failedFileCount = 0;
    constructor(config, db, dbUri, _projectId) {
        super();
     ...
     */
    async processFile(filePath, precomputedHash) {
        ...
        if (SKIP_EXTS.has(extname(filePath).toLowerCase()))
            return;
        if (SKIP_DIRS.some(d => filePath.includes(d)))
            return;
    ...
    async search(query, options = {}) {
        ...
        for (const entry of entries) {
            ...
            if (options.filters?.paths?.length) {
                const matchesPath = options.filters.paths.some(p => entry.sourcePath?.startsWith(p));
                if (!matchesPath)
                    continue;
            }
            // Deduplicate by file path
            if (entry.sourcePath && seenPaths.has(entry.sourcePath))
                continue;
            if (entry.sourcePath)
    ...
    getStats() {
        ...
        return {
            ...
            totalChunks: Array.from(allFiles.values()).reduce((sum, f) => sum + f.chunkCount, 0),
 ...
 */
export function createIndexer(config, db, dbUri) {
